  ...
  <div class="knowledge-manage-container p-5 bg-background-DEFAULT min-h-[calc(100vh-84px)]">
    ...
    <Card class="mb-5" elevation="small" :maxWidth="false">
      <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div class="flex items-center gap-2 flex-wrap">
          <InputField v-model="searchModel.sportType" placeholder="输入运动类型搜索" :leftIcon="Search" size="medium"
            class="w-full md:w-60" @keyup.enter="getSportInfoList" />
    ...
    <Card elevation="small" :maxWidth="false">
      ...
      <div v-else-if="sportInfoList.length > 0" class="overflow-x-auto">
        <table class="w-full table-auto border-collapse text-left">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="sport in sportInfoList" :key="sport.id"
              ...
              <td class="px-4 py-3 text-center align-middle">
                <div class="flex justify-center items-center gap-2">
                  <Button @click="openDialog(sport.id)" type="primary" :icon="Edit" size="small" iconOnly
                    tooltip="编辑"></Button>
                  <Button @click="deleteSportInfo(sport)" type="danger" :icon="Delete" size="small" iconOnly
                    tooltip="删除"></Button>
                </div>
              </td>
            </tr>
      ...
      <div v-if="total > 0 && !listLoading" class="mt-5 flex flex-col md:flex-row justify-between items-center">
        ...
        <div class="flex items-center gap-2">
          ...
          <select v-model="searchModel.pageSize" @change="getSportInfoList"
    ...
    <Modal v-model="dialogFormVisible" :title="dialogTitle" size="lg" backdropStyle="glass">
      ...
      <form @submit.prevent="saveSportInfo" class="space-y-4">
        <div>
          <label for="sportType" class="block text-sm font-medium text-text-secondary mb-1">运动类型 <span
              class="text-red-500">*</span></label>
          <InputField id="sportType" v-model="sportInfoForm.sportType" placeholder="例如：跑步、游泳、瑜伽"
            :error="!!formErrors.sportType" :errorMessage="formErrors.sportType" required />
        </div>
        <div>
          <label for="suitableFrequency" class="block text-sm font-medium text-text-secondary mb-1">运动频率 <span
              class="text-red-500">*</span></label>
          <InputField id="suitableFrequency" v-model="sportInfoForm.suitableFrequency" placeholder="例如：每周 3-5 次"
            :error="!!formErrors.suitableFrequency" :errorMessage="formErrors.suitableFrequency" required />
        </div>
        <div>
          <label for="suitableTime" class="block text-sm font-medium text-text-secondary mb-1">运动时间 <span
              class="text-red-500">*</span></label>
          <InputField id="suitableTime" v-model="sportInfoForm.suitableTime" placeholder="例如：每次 30-45 分钟"
            :error="!!formErrors.suitableTime" :errorMessage="formErrors.suitableTime" required />
        </div>
        <div>
          <label for="recommendedSpeed" class="block text-sm font-medium text-text-secondary mb-1">运动速度/强度 <span
              class="text-red-500">*</span></label>
          <InputField id="recommendedSpeed" v-model="sportInfoForm.recommendedSpeed" placeholder="例如：中等强度，保持微喘"
            :error="!!formErrors.recommendedSpeed" :errorMessage="formErrors.recommendedSpeed" required />
        </div>
        <div>
          <label for="suitableHeartRate" class="block text-sm font-medium text-text-secondary mb-1">运动心率 <span
              class="text-red-500">*</span></label>
          <InputField id="suitableHeartRate" v-model="sportInfoForm.suitableHeartRate" placeholder="例如：120-150 次/分钟"
            :error="!!formErrors.suitableHeartRate" :errorMessage="formErrors.suitableHeartRate" required />
        </div>
...
// --- Icon Imports ---
import { Search, Refresh, Plus, Edit, Delete, Weightlifting } from '@icon-park/vue-next';
...
const validateForm = (): boolean => {
  ...
  fields.forEach(field => {
    if (!sportInfoForm[field]?.trim()) {
      ...
      formErrors[field] = `${labelMap[String(field)] || String(field)}不能为空`;
      isValid = false;
    }
  });
  return isValid;
...
const getSportInfoList = async () => {
  ...
  saveMessage.value = null;
  try {
    // knowledgeManageApi.getSportList() 返回的已经是 {rows, total} 对象
    const sportDataPayload = await knowledgeManageApi.getSportList(searchModel);

    // 直接检查 sportDataPayload 的属性
    if (sportDataPayload && typeof sportDataPayload === 'object' &&
        'rows' in sportDataPayload && Array.isArray(sportDataPayload.rows) &&
        'total' in sportDataPayload && typeof sportDataPayload.total === 'number') {
      sportInfoList.value = sportDataPayload.rows;
      total.value = sportDataPayload.total;
    } else {
      console.error("Unexpected response structure from getSportList (expected {rows, total}):", sportDataPayload);
      sportInfoList.value = [];
      total.value = 0;
...
const openDialog = async (id?: number) => {
  ...
  if (id) {
    ...
    try {
      // knowledgeManageApi.getSportInfoById(id) 返回的已经是 SportInfo 对象 (拦截器处理后)
      const sportInfoData = await knowledgeManageApi.getSportInfoById(id);

      // 直接检查 sportInfoData 的属性
      if (sportInfoData && typeof sportInfoData === 'object' && 'id' in sportInfoData) {
        Object.assign(sportInfoForm, sportInfoData);
      } else {
        console.error("Invalid data received for sport info (expected SportInfo object):", sportInfoData);
