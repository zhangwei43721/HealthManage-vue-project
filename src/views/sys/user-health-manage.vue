  ...
  <div class="user-health-manage-container p-5 bg-background-DEFAULT min-h-[calc(100vh-84px)]">
    ...
    <Card class="mb-5" elevation="small" :maxWidth="false">
      <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div class="flex items-center gap-3 flex-wrap">
          <InputField v-model="searchModel.name" placeholder="输入用户姓名" :leftIcon="UserIcon" size="medium"
            class="w-full sm:w-auto" @keyup.enter="getBodyDataList" />
          <InputField v-model="searchModel.id" placeholder="输入用户ID" type="number" :leftIcon="IdCardH" size="medium"
            class="w-full sm:w-auto" @keyup.enter="getBodyDataList" />
    ...
    <Card elevation="small" :maxWidth="false">
      ...
      <div v-else-if="bodyDataList.length > 0" class="overflow-x-auto">
        <table class="w-full table-auto border-collapse text-left text-sm">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="bodyData in bodyDataList" :key="bodyData.id"
              ...
              <td class="p-3 text-center align-middle">
                <span
                  :class="['px-2 py-0.5 rounded-full text-xs font-medium', bodyData.smoking ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700']">
                  {{ bodyData.smoking ? '是' : '否' }}
                </span>
              </td>
              <td class="p-3 text-center align-middle">
                <span
                  :class="['px-2 py-0.5 rounded-full text-xs font-medium', bodyData.drinking ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700']">
                  {{ bodyData.drinking ? '是' : '否' }}
                </span>
              </td>
              <td class="p-3 text-center align-middle">
                <span
                  :class="['px-2 py-0.5 rounded-full text-xs font-medium', bodyData.exercise ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700']">
                  {{ bodyData.exercise ? '是' : '否' }}
                </span>
              </td>
              <td class="p-3 text-center align-middle">
                <div class="flex justify-center items-center gap-2">
                  <Button @click="openDialog(bodyData.id)" type="primary" :icon="Edit" size="small" iconOnly
                    tooltip="编辑"></Button>
                  <Button @click="confirmDelete(bodyData)" type="danger" :icon="Delete" size="small" iconOnly
                    tooltip="删除"></Button>
      ...
      <div v-if="total > 0 && !listLoading" class="mt-5 flex flex-col md:flex-row justify-between items-center">
        ...
        <div class="flex items-center gap-2">
          ...
          <select v-model="searchModel.pageSize" @change="getBodyDataList"
    ...
    <Modal v-model="dialogFormVisible" :title="dialogTitle" size="xl" backdropStyle="glass">
      ...
      <form @submit.prevent="saveBodyData" class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
        ...
        <div>
          <label for="bodyName" class="block text-sm font-medium text-text-secondary mb-1">姓名 <span
              class="text-red-500">*</span></label>
          <InputField id="bodyName" v-model="bodyForm.name" placeholder="用户姓名" :error="!!formErrors.name"
            :errorMessage="formErrors.name" required />
        </div>
        <div>
          <label for="bodyAge" class="block text-sm font-medium text-text-secondary mb-1">年龄 <span
              class="text-red-500">*</span></label>
          <InputField id="bodyAge" v-model.number="bodyForm.age" type="number" placeholder="年龄"
            :error="!!formErrors.age" :errorMessage="formErrors.age" required />
        </div>
        <div>
          <label for="bodyGender" class="block text-sm font-medium text-text-secondary mb-1">性别 <span
              class="text-red-500">*</span></label>
          <select id="bodyGender" v-model="bodyForm.gender"
        ...
        <div>
          <label for="bodyHeight" class="block text-sm font-medium text-text-secondary mb-1">身高 (cm) <span
              class="text-red-500">*</span></label>
          <InputField id="bodyHeight" v-model.number="bodyForm.height" type="number" placeholder="身高"
            :error="!!formErrors.height" :errorMessage="formErrors.height" required />
        </div>
        <div>
          <label for="bodyWeight" class="block text-sm font-medium text-text-secondary mb-1">体重 (kg) <span
              class="text-red-500">*</span></label>
          <InputField id="bodyWeight" v-model.number="bodyForm.weight" type="number" placeholder="体重"
            :error="!!formErrors.weight" :errorMessage="formErrors.weight" required />
        </div>
        <div>
          <label for="bodyHeartRate" class="block text-sm font-medium text-text-secondary mb-1">心率 (次/分) <span
              class="text-red-500">*</span></label>
          <InputField id="bodyHeartRate" v-model.number="bodyForm.heartRate" type="number" placeholder="静息心率"
            :error="!!formErrors.heartRate" :errorMessage="formErrors.heartRate" required />
        </div>
        <div>
          <label for="bodyVision" class="block text-sm font-medium text-text-secondary mb-1">视力 (5.0记为50) <span
              class="text-red-500">*</span></label>
          <InputField id="bodyVision" v-model.number="bodyForm.vision" type="number" placeholder="视力"
            :error="!!formErrors.vision" :errorMessage="formErrors.vision" required />
        </div>

        <!-- Health Indicators -->
        <div class="md:col-span-2 font-medium text-text-primary border-b pb-1 mb-2 mt-4">健康指标</div>
        <div>
          <label for="bodyBloodSugar" class="block text-sm font-medium text-text-secondary mb-1">血糖 (mmol/L) <span
              class="text-red-500">*</span></label>
          <InputField id="bodyBloodSugar" v-model.number="bodyForm.bloodSugar" type="number" step="0.1"
            placeholder="血糖值" :error="!!formErrors.bloodSugar" :errorMessage="formErrors.bloodSugar" required />
        </div>
        <div>
          <label for="bodyBloodPressure" class="block text-sm font-medium text-text-secondary mb-1">血压 (mmHg) <span
              class="text-red-500">*</span></label>
          <InputField id="bodyBloodPressure" v-model="bodyForm.bloodPressure" placeholder="例如: 120/80"
            :error="!!formErrors.bloodPressure" :errorMessage="formErrors.bloodPressure" required />
        </div>
        <div>
          <label for="bodyBloodLipid" class="block text-sm font-medium text-text-secondary mb-1">血脂 <span
              class="text-red-500">*</span></label>
          <InputField id="bodyBloodLipid" v-model="bodyForm.bloodLipid" placeholder="例如: 正常 / 偏高"
            :error="!!formErrors.bloodLipid" :errorMessage="formErrors.bloodLipid" required />
        </div>

        <!-- Lifestyle -->
        <div class="md:col-span-2 font-medium text-text-primary border-b pb-1 mb-2 mt-4">生活习惯</div>
        <div>
          <label for="bodySleepDuration" class="block text-sm font-medium text-text-secondary mb-1">睡眠时长 (h) <span
              class="text-red-500">*</span></label>
          <InputField id="bodySleepDuration" v-model.number="bodyForm.sleepDuration" type="number" step="0.5"
            placeholder="小时" :error="!!formErrors.sleepDuration" :errorMessage="formErrors.sleepDuration" required />
        </div>
        <div>
          <label for="bodySleepQuality" class="block text-sm font-medium text-text-secondary mb-1">睡眠质量 <span
              class="text-red-500">*</span></label>
          <InputField id="bodySleepQuality" v-model="bodyForm.sleepQuality" placeholder="例如: 良好 / 一般 / 差"
            :error="!!formErrors.sleepQuality" :errorMessage="formErrors.sleepQuality" required />
        </div>
        <div>
          <label for="bodyWaterConsumption" class="block text-sm font-medium text-text-secondary mb-1">日均饮水量 (L) <span
              class="text-red-500">*</span></label>
          <InputField id="bodyWaterConsumption" v-model.number="bodyForm.waterConsumption" type="number" step="0.1"
            placeholder="升" :error="!!formErrors.waterConsumption" :errorMessage="formErrors.waterConsumption"
            required />
        </div>
        <div>
          <label for="bodyFoodTypes" class="block text-sm font-medium text-text-secondary mb-1">主要食物类型</label>
          <InputField id="bodyFoodTypes" v-model="bodyForm.foodTypes" placeholder="简单描述饮食习惯"
            :error="!!formErrors.foodTypes" :errorMessage="formErrors.foodTypes" />
        </div>
        <div class="flex items-center gap-4">
          <label class="flex items-center">
            <input type="checkbox" v-model="bodyForm.smoking"
              class="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary" />
            <span class="ml-2 text-sm text-text-secondary">是否吸烟</span>
          </label>
          <label class="flex items-center">
            <input type="checkbox" v-model="bodyForm.drinking"
              class="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary" />
            <span class="ml-2 text-sm text-text-secondary">是否饮酒</span>
          </label>
          <label class="flex items-center">
            <input type="checkbox" v-model="bodyForm.exercise"
              class="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary" />
...
import type { ListParams } from '@/services/userHealthManage';

// Define search parameter type
interface BodySearchParams {
  name?: string;
  id?: number | string; // Allow string initially from input
  pageNo: number;
  pageSize: number;
}
...
// --- Icon Imports ---
import { Search, Refresh, Edit, Delete, ChartHistogram, User as UserIcon, IdCardH } from '@icon-park/vue-next';
...
const validateForm = (): boolean => {
  ...
  requiredFields.forEach(field => {
    ...
    if (value === undefined || value === null || String(value).trim() === '') {
      ...
      formErrors[field] = `${labelMap[field as keyof typeof labelMap] || String(field)}不能为空`;
      isValid = false;
    }
...
const getBodyDataList = async () => {
  ...
  saveMessage.value = null;
  try {
    const params: Partial<BodySearchParams> = { ...searchModel };
    ...
    // userHealthManageApi.getAllBodyList() 返回的已经是 {rows, total} 对象
    const bodyDataPayload = await userHealthManageApi.getAllBodyList(params as ListParams);

    // 直接检查 bodyDataPayload 的属性
    if (bodyDataPayload && typeof bodyDataPayload === 'object' &&
        'rows' in bodyDataPayload && Array.isArray(bodyDataPayload.rows) &&
        'total' in bodyDataPayload && typeof bodyDataPayload.total === 'number') {
      bodyDataList.value = bodyDataPayload.rows;
      total.value = bodyDataPayload.total;
    } else {
      console.error("Unexpected response structure from getAllBodyList (expected {rows, total}):", bodyDataPayload);
      bodyDataList.value = [];
      total.value = 0;
...
const openDialog = async (id?: number) => {
  ...
  try {
    // userHealthManageApi.getBodyById(id) 现在直接返回 Body 对象
    const formData = await userHealthManageApi.getBodyById(id);
    ...
    if (formData && typeof formData === 'object' && 'id' in formData) {
      ...
      console.error("Invalid data received for body data (expected Body object):", formData);
...
const saveBodyData = async () => {
  if (!validateForm() || !bodyForm.id) {
    return;
  }
  saveLoading.value = true;
  saveMessage.value = null;
  try {
    // userHealthManageApi.updateBody(bodyForm as Body) 预计不返回特定业务数据或由拦截器处理
    // 如果API设计为成功时不返回内容，或者返回简单状态码，这里不需要读取 result.data
...
const confirmDelete = async (bodyData: Body) => {
  if (window.confirm(`确定要删除用户【${bodyData.name}】(ID: ${bodyData.id}) 的这条健康数据吗？`)) {
    ...
    saveMessage.value = null;
    try {
      // userHealthManageApi.deleteBodyById(bodyData.id) 预计不返回特定业务数据或由拦截器处理
      ...
      await getBodyDataList(); // Refresh the list
    } catch (error) {
      ...
      // Ensure listLoading is reset even if getBodyDataList itself handles it,
      // in case of an error before getBodyDataList is called or if it doesn't run.
      listLoading.value = false;
    }
  }
};
