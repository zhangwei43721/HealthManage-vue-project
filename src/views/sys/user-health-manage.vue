  ...
  try {
    ...
    if (params.id && typeof params.id === 'string') {
      ...
      if (!isNaN(parsedId)) {
        ...
        delete params.id; // Remove if not a valid number string
      }
    } else if (params.id && typeof params.id !== 'number') {
      delete params.id; // Clean up if it's neither string nor number
    }

    const response = await userHealthManageApi.getAllBodyList(params as ListParams); // Cast to expected type for API
