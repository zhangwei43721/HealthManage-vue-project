...
const getUserList = async () => {
  ...
  try {
    // userApi.getUserList() 返回的已经是 {rows, total} 对象 (拦截器处理后)
    const userDataPayload = await userApi.getUserList(searchModel);

    // 直接检查 userDataPayload 的属性
    if (userDataPayload && typeof userDataPayload === 'object' &&
        'rows' in userDataPayload && Array.isArray(userDataPayload.rows) &&
        'total' in userDataPayload && typeof userDataPayload.total === 'number') {
      userList.value = userDataPayload.rows;
      total.value = userDataPayload.total;
    } else {
      console.error("Unexpected response structure from getUserList (expected {rows, total}):", userDataPayload);
      userList.value = [];
      total.value = 0;
      saveMessage.value = { type: 'error', text: '获取用户列表失败: 响应格式不正确' }; // 更具体的错误信息
    }
  } catch (error) {
...
const getAllRoles = async () => {
  try {
    // roleApi.getAllRoles() 现在直接返回 Role[] (如果API直接返回数组)
    const rolesArray = await roleApi.getAllRoles();

    // 直接检查 rolesArray 是否是一个数组
    if (rolesArray && Array.isArray(rolesArray)) {
      allRoleList.value = rolesArray;
    } else {
      // 这种情况理论上不应该发生，除非API返回了非数组或者null/undefined
      // 或者如果API确实是 {code, data:[...]} 结构，那么 roleApi.getAllRoles()会返回 data 部分的数组
      // 这里的 console.error 更多的是一个保险措施
      console.error("Invalid response structure received by component from roleApi.getAllRoles (expected array):", rolesArray);
      allRoleList.value = [];
      // 可以选择性地显示一个用户提示
      // saveMessage.value = { type: 'error', text: '获取角色列表失败: 响应格式不正确' };
    }
  } catch (error) {
    console.error("Error fetching roles in component:", error);
    allRoleList.value = []; // 清空列表
    // saveMessage.value = { type: 'error', text: '获取角色列表失败' }; // 也可以在这里设置错误消息
  }
};
...
const openDialog = async (id: number | null | undefined) => {
  ...
  if (id) {
    ...
    try {
      // userApi.getUserById(id) 返回的已经是 User 对象的部分或全部 (拦截器处理后)
      const userData = await userApi.getUserById(id);

      // 直接检查 userData 的属性
      if (userData && typeof userData === 'object' && ('id' in userData || 'username' in userData) ) { // 检查id或username确保是用户对象
        Object.assign(userForm, userData);
        userForm.password = '';
        userForm.roleIdList = (userData as any).roleIdList || []; // 确保 roleIdList 是数组
      } else {
        console.error("Invalid data received for user (expected User object):", userData);
        saveMessage.value = { type: 'error', text: '获取用户信息失败: 数据格式无效' };
        return;
      }
    } catch (error) {
...
const saveUser = async () => {
  ...
  saveMessage.value = null;
  try {
    let result: { code?: number; message?: string; status?: number; /* other possible fields */ }; // Adjusted type for flexibility

    if (isEditMode.value) {
      const updateData = { ...userForm };
      delete updateData.password;
      if (updateData.id !== undefined) {
        result = await userApi.updateUser(updateData); // result is the payload from interceptor
      } else {
        saveMessage.value = { type: 'error', text: '用户ID不能为空' };
        saveLoading.value = false; // Ensure loading is stopped
        return;
      }
    } else {
      const addData = { ...userForm };
      delete addData.id;
      result = await userApi.addUser(addData); // result is the payload from interceptor
    }
  ...
  } catch (error: any) { // Catch block handles rejected promises from api calls or interceptor
    console.error("Save user error:", error);
    saveMessage.value = { type: 'error', text: error?.message || `操作失败: 未知错误` };
  } finally {
...
const deleteUser = async (user: User) => {
  if (window.confirm(`您确定要删除用户【${user.username}】吗？`)) {
    ...
    try {
      ...
      if (result && (result as any).code === 20000) {
        saveMessage.value = { type: 'success', text: (result as any).message || '用户删除成功' };
      } else if (result === undefined || result === null) {
         saveMessage.value = { type: 'success', text: '用户删除成功' };
      }
      else {
        saveMessage.value = { type: 'error', text: (result as any)?.message || '删除失败' };
      }
      ...
      saveMessage.value = { type: 'error', text: error?.message || `删除用户时出错: 未知错误` };
    } finally {
      listLoading.value = false;
    }
  }
