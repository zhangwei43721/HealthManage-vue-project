  ...
  <div class="sport-detail-manage-container p-5 bg-background-DEFAULT min-h-[calc(100vh-84px)]">
      ...
      <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div class="flex items-center gap-2 flex-wrap">
          <InputField v-model="searchModel.sportType" placeholder="输入运动类型搜索" :leftIcon="Search" size="medium"
            class="w-full md:w-60" @keyup.enter="getDetailList" />
    ...
    <Card elevation="small" :maxWidth="false">
        ...
        <table class="w-full table-auto border-collapse text-left">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="detail in detailList" :key="detail.id"
              ...
              <td class="px-4 py-3 text-sm text-text-secondary align-middle max-w-xs truncate" :title="detail.disease">
                {{ detail.disease }}</td>
              <td class="px-4 py-3 text-sm text-text-secondary align-middle max-w-sm truncate" :title="detail.method">{{
                detail.method }}</td>
              <td class="px-4 py-3 text-sm text-text-secondary align-middle max-w-xs truncate" :title="detail.notes">{{
                detail.notes }}</td>
              <td class="px-4 py-3 text-center align-middle">
                <div class="flex justify-center items-center gap-2">
                  <Button @click="openDialog(detail.id)" type="primary" :icon="Edit" size="small" iconOnly
                    tooltip="编辑"></Button>
                  <Button @click="deleteDetail(detail)" type="danger" :icon="Delete" size="small" iconOnly
                    tooltip="删除"></Button>
                </div>
              </td>
            </tr>
      ...
      <div v-if="total > 0 && !listLoading" class="mt-5 flex flex-col md:flex-row justify-between items-center">
        ...
        <div class="flex items-center gap-2">
          ...
          <select v-model="searchModel.pageSize" @change="getDetailList"
    ...
    <Modal v-model="dialogFormVisible" :title="dialogTitle" size="lg" backdropStyle="glass">
      ...
      <form @submit.prevent="saveDetail" class="space-y-4">
        <div>
          <label for="detailSportType" class="block text-sm font-medium text-text-secondary mb-1">运动类型 <span
              class="text-red-500">*</span></label>
          <InputField id="detailSportType" v-model="detailForm.sportType" placeholder="例如：跑步、游泳"
        ...
        <div>
          ...
          <InputField id="detailDisease" v-model="detailForm.disease" placeholder="例如：心脏病、高血压患者不宜剧烈运动" type="textarea"
            :rows="3" :error="!!formErrors.disease" :errorMessage="formErrors.disease" />
        </div>
        <div>
          <label for="detailMethod" class="block text-sm font-medium text-text-secondary mb-1">运动方法 <span
              class="text-red-500">*</span></label>
          <InputField id="detailMethod" v-model="detailForm.method" placeholder="详细描述运动步骤、技巧等" type="textarea" :rows="5"
            :error="!!formErrors.method" :errorMessage="formErrors.method" required />
        </div>
        <div>
          <label for="detailNotes" class="block text-sm font-medium text-text-secondary mb-1">注意事项</label>
          <InputField id="detailNotes" v-model="detailForm.notes" placeholder="例如：运动前热身、运动后拉伸" type="textarea" :rows="4"
            :error="!!formErrors.notes" :errorMessage="formErrors.notes" />
        </div>
...
// --- Icon Imports ---
import { Search, Refresh, Plus, Edit, Delete, Clipboard } from '@icon-park/vue-next';
...
const getDetailList = async () => {
  ...
  saveMessage.value = null;
  try {
    // sportItemManageApi.getDetailList() 返回的已经是 {rows, total} 对象
    const detailDataPayload = await sportItemManageApi.getDetailList(searchModel);

    // 直接检查 detailDataPayload 的属性
    if (detailDataPayload && typeof detailDataPayload === 'object' &&
        'rows' in detailDataPayload && Array.isArray(detailDataPayload.rows) &&
        'total' in detailDataPayload && typeof detailDataPayload.total === 'number') {
      detailList.value = detailDataPayload.rows;
      total.value = detailDataPayload.total;
    } else {
      console.error("Unexpected response structure from getDetailList (expected {rows, total}):", detailDataPayload);
      detailList.value = [];
      total.value = 0;
...
const openDialog = async (id?: number) => {
  ...
  if (id) {
    ...
    try {
      // sportItemManageApi.getDetailById(id) 返回的已经是 Detail 对象 (拦截器处理后)
      const detailData = await sportItemManageApi.getDetailById(id);

      // 直接检查 detailData 的属性
      if (detailData && typeof detailData === 'object' && 'id' in detailData) {
        Object.assign(detailForm, detailData);
      } else {
        console.error("Invalid data received for sport detail (expected Detail object):", detailData);
...
const deleteDetail = async (detail: Detail) => {
  if (window.confirm(`确定要删除运动类型【${detail.sportType}】的详情吗？`)) {
    ...
    saveMessage.value = null;
    try {
      await sportItemManageApi.deleteDetail(detail.id);
...
.sport-detail-manage-container :deep(textarea) {
  min-height: 80px;
  /* Adjust min height for textareas */
}
</style>
