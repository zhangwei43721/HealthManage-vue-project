...
const fetchData = async () => {
  ...
  try {
    ...
    const response = await healthApi.getUserBodyList(params);

    if (response.code === 20000 && response.data) {
      const processedRows = response.data.rows.filter((item: BodyNotes) =>
        !searchKeyword.value ||
        ...
        item.sleepQuality = item.sleepQuality !== null && item.sleepQuality !== undefined ? Number(item.sleepQuality) : null;
        if (isNaN(item.sleepQuality as number)) {
          if (item.sleepQuality === '好') item.sleepQuality = 1;
          else if (item.sleepQuality === '一般') item.sleepQuality = 2;
          else if (item.sleepQuality === '差') item.sleepQuality = 3;
          else item.sleepQuality = null;
        }
        return item;
      });

      logList.value = processedRows;
      total.value = response.data.total ?? 0;

    } else {
      logList.value = [];
      total.value = 0;
      showNotification('error', response.message || '获取数据失败');
    }
  } catch (error) {
    console.error('获取健康日志失败:', error);
    const errorMsg = (error instanceof Error) ? error.message : '获取健康日志失败';
...
const handleDelete = (item: BodyNotes) => {
  if (!item.notesid) {
    showNotification('error', '无效的记录 ID');
    return;
  }

  showConfirm(`确认删除姓名为 "${item.name}" 的这条健康记录吗？`, async () => {
    try {
      const response = await healthApi.deleteUserBodyById(item.notesid!);
      if (response.code === 20000) {
        showNotification('success', response.message || '删除成功');
        fetchData();
      } else {
