...
const fetchCurrentPageSuggestion = async () => {
  ...
  if (!type || !isLoggedIn.value) return;

  loading.value = true;
  errorMessage.value = '';

  try {
    // The api service interceptor handles the { code: 20000, data: ... } structure
    const response = await api.get(`/ai-suggestions-specific/${type}`);

    // Check if response has expected structure
    if (
      response?.data?.suggestion !== undefined &&
      response?.data?.generatedAt !== undefined
    ) {
      currentSuggestion.value = response.data.suggestion || '';

      // Handle date format (array or ISO string)
      generatedAt.value = Array.isArray(response.data.generatedAt)
        ? new Date(
            response.data.generatedAt[0],
            response.data.generatedAt[1] - 1,
            response.data.generatedAt[2],
            response.data.generatedAt[3] || 0,
            response.data.generatedAt[4] || 0,
            response.data.generatedAt[5] || 0
          ).toISOString()
        : response.data.generatedAt || new Date().toISOString();

      // Set notification flag if collapsed
      if (!expanded.value) {
